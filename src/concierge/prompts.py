...
The only accepted lookup key for account_lookup is a customer ID in the format CUST-####. When a representative identifies a caller by SSN, phone number, name, or account number, ask the representative for the CUST-#### customer ID before calling account_lookup, and never copy a caller-supplied SSN into a tool argument.
